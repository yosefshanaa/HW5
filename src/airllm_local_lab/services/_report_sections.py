...
def section_models() -> str:
    return (
        ...
        "| Giant OOM proof | `huggyllama/llama-13b` | 26 GB | "
        "Apache-2.0, ungated, safetensors-native. 26 GB FP16 >> 18 GB RAM. "
        "Real OOM confirmed (180s timeout). AirLLM streaming fails: LLaMA-1 arch "
        "incompatible with `AirLLMLlamaMlx` (LLaMA-2 weight layout). |\n"
        "| AirLLM demo + TPOT sweep | `TinyLlama/TinyLlama-1.1B-Chat-v1.0` | ~2.2 GB | "
        "LLaMA-2 architecture; compatible with `AirLLMLlamaMlx` on macOS; "
        "confirms layer-streaming at 1416 ms/token. |\n"
        ...
        "**Negative result (LLaMA-1 vs LLaMA-2):** `huggyllama/llama-13b` uses original LLaMA-1 weight names "
        "(e.g. `rotary_emb` in attention). `AirLLMLlamaMlx` targets LLaMA-2 layout and fails on LLaMA-1 "
        "with `Module does not have parameter named rotary_emb`. Direct OOM proof remains valid. "
        "AirLLM streaming is confirmed by TinyLlama (LLaMA-2 arch, 1416 ms/token)."
    )
