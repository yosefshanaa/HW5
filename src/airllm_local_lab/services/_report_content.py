...
def section_run_instructions() -> str:
    return (
        ...
        "Run `uv run chat` for a live REPL loop — the model loads once and stays resident "
        "between turns, keeping the last 3 exchanges in context.\n\n"
        ...
        "*S3 — A follow-up turn in the same session: the user asks for examples of AI in healthcare. "
        "The model delivers a structured multi-point answer (predictive analytics, diagnostic imaging) "
        "demonstrating that conversation context is correctly preserved across turns. "
        "A second full 22-layer streaming pass is visible, confirming each generation is independent "
        "NVMe-to-RAM demand paging — no weights remain cached in Python memory between calls.*"
    )
